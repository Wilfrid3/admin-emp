    ...
    <div class="d-flex flex-column flex-column-fluid">
        ...
        <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
            <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-stack">
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <!--begin::Title-->
                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Modifier un article</h1>
                    <!--end::Title-->
                    <!--begin::Breadcrumb-->
                ...
                <div class="d-flex align-items-center gap-2 gap-lg-3">
                    <a href="{{route('articles')}}" class="btn btn-sm fw-bold btn-dark">Retour</a>
                </div>
                <!--end::Actions-->
            </div>
        </div>
        ...
        <div id="kt_app_content" class="app-content flex-column-fluid">
            ...
            <div id="kt_app_content_container" class="app-container container-xxl">
                <form class="form d-flex flex-column flex-lg-row" action="{{ route('article-update', ['id'=>$article->id]) }}" method="POST"  enctype="multipart/form-data">
                    ...
                    <div class="d-flex flex-column gap-7 gap-lg-10 w-100 w-lg-300px mb-7 me-lg-10">
                        <div class="card card-flush py-4">
                            ...
                            <div class="card-body text-center pt-0">
                                <style>.image-input-placeholder { background-image: url({{asset('storage/'.$article->lienImg)}}); } [data-theme="dark"] .image-input-placeholder { background-image: url({{asset('storage/'.$article->lienImg)}}); }</style>
                                <div class="image-input image-input-empty image-input-outline image-input-placeholder mb-3" data-kt-image-input="true">
                                    <div class="image-input-wrapper w-150px h-150px"></div>
                                    <label class="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-body shadow" data-kt-image-input-action="change" data-bs-toggle="tooltip" title="Ajouter une photo">
                                        <i class="bi bi-camera-fill fs-7"></i>
                                        <input type="file" name="lienImg" accept=".png, .jpg, .jpeg" />
                                        <input type="hidden" name="old_lienImg"  value="{{$article->lienImg}}"/>
                                        <input type="hidden" name="idCompte" value="{{$data->id}}" />
                                    </label>
                                    <span class="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-body shadow" data-kt-image-input-action="cancel" data-bs-toggle="tooltip" title="Annuler la photo">
                                ...
                                <span class="text-danger">@error('lienImg') {{ $message }} @enderror</span>
                                <div class="text-muted fs-7">Selectionnez la photo principale de l`article. Uniquement les images *.png, *.jpg et *.jpeg sont autorisés</div>
                            </div>
                        </div>
                        <div class="card card-flush py-4">
                            <div class="card-header">
                        ...
                        <div class="card card-flush py-4">
                            ...
                            <div class="card-body pt-0">
                                <label class="required form-label">Type d`article</label>
                                <select class="form-select mb-2" data-control="select2" data-placeholder="-- Selectionnez --" data-allow-clear="true" name="idTypearticle">
                                    <option></option>
                                    @foreach ($typearticles as $typearticle)
                                        <option value="{{$typearticle->id}}" {{($typearticle->id == $article->idTypearticle) ? 'selected' : ''}} >{{$typearticle->libelle}}</option>
                                    @endforeach
                                </select>
                                <span class="text-danger">@error('idTypearticle') {{ $message }} @enderror</span>
                                <div class="text-muted fs-7 mb-7">Type d`article obligatoire.</div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div class="card card-flush py-4">
                                        ...
                                        <div class="card-body pt-0">
                                            <div class="mb-10 fv-row">
                                                <label class="required form-label">Libelle de l`article</label>
                                                <input type="text" name="libelle" class="form-control mb-2" placeholder="Entrez le libelle de l`article" value="{{$article->libelle}}" />
                                                <span class="text-danger">@error('libelle') {{ $message }} @enderror</span>
                                                <div class="text-muted fs-7">Le libelle de l`article est obligatoire et doit être unique.</div>
                                            </div>
                                            <div class="mb-10 row">
                                                <div class="col-md-6 mb-3">
                                                    <label class="required form-label">Quantité</label>
                                                    <input type="number" name="quantite" class="form-control mb-2" placeholder="Entrez la quantité" value="{{$article->quantite}}" />
                                                    <span class="text-danger">@error('quantite') {{ $message }} @enderror</span>
                                                    <div class="text-muted fs-7">La quantité est obligatoire</div>
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label class="required form-label">Prix</label>
                                                    <input type="number" name="prix" class="form-control mb-2" placeholder="Entrez le prix de l`article" value="{{$article->prix}}" />
                                                    <span class="text-danger">@error('prix') {{ $message }} @enderror</span>
                                                    <div class="text-muted fs-7">Le prix actuel de l`article est obligatoire.</div>
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label class="required form-label">Ancien prix</label>
                                                    <input type="number" name="oldPrix" class="form-control mb-2" placeholder="Entrez l'ancien prix de l`article" value="{{$article->oldPrix}}" />
                                                    <span class="text-danger">@error('oldPrix') {{ $message }} @enderror</span>
                                                    <div class="text-muted fs-7">L'ancien prix de l`article est obligatoire.</div>
                                                </div>
                                                <div class="col-md-6 mb-3">
                                                    <label class="form-label">Remise</label>
                                                    <input type="number" name="remise" class="form-control mb-2" placeholder="Entrez la remise" value="{{$article->remise}}" />
                                                    <div class="text-muted fs-7">La remise en %.</div>
                                                </div>
                                            </div>
                                            <div class="mb-11">
                                                <label class="required form-label">Description</label>
                                                <textarea name="description" class="form-control min-h-100px mb-2" placeholder="Entrez la description">{{$article->description}}</textarea>
                                                <span class="text-danger">@error('description') {{ $message }} @enderror</span>
                                                <div class="text-muted fs-7">Ajoutez une description à cet article.</div>
                                            </div>
